...
# Исследование данных проекта Permafrost Analysis

## 📊 Полный обзор данных проекта

### **1. CALM Site Data (СТС.csv) — Многолетняя мерзлота**

Это глобальная сеть **Circumpolar Active Layer Monitoring** с площадок по всему Северу России, Сибири, Чукотке.

**Что хранится:**
- `Site Code` (R1-R43): идентификаторы площадок (R42 = **Туймада, Якутск** — ваш основной объект)
- Координаты (LAT/LONG)
- **ALT (Active Layer Thickness)** в см для каждого года с 1990 по 2024
- **Method**: тип измерения (50/TT, 100, 10/T, etc.)

**Ключевые наблюдения R42 (Tuymada, Yakutsk)**:
```
R42: Tuymada (Yakutsk region) - 62.0133°N, 129.657°E
Method: 50/TT (сетка 50м × 50м, ежегодные измерения)
ALT с 2005-2024: колеблется 201-208 см (очень стабильно!)
```

---

### **2. Метеорологические аномалии (3 файла)**

#### **A. Monthly Anomalies** (`monthly_anomalies_operational.csv`)
Выявляет месячные экстремумы и их связь с мерзлотой:

| Параметр | Значение |
|----------|----------|
| **Сигналы** | аномально тёплый месяц, аномально влажный месяц, много дней оттепели |
| **Z-score** | стандартизация отклонения от нормы (>2 = экстремум) |
| **Risk Points** | 1-2 балла за каждый триггер |
| **Severity** | умеренная vs экстремальная |

Примеры: 
- 2008-02, Жиганск: экстремально влажный февраль (precip_z = 3.32)
- 2009-06, Оймякон: экстремально тёплый июнь (Z = 3.02)
- 2009-10, Оймякон: аномально много дней оттепели (Z = 6.78!) — **ЭКСТРЕМАЛЬНО**

#### **B. Daily Anomalies** (`daily_anomalies_operational.csv`)
Суточный мониторинг:
- Резкие потепления за сутки (ΔT ≥ 8°C)
- Экстремальные суточные осадки
- "Дни опасной погоды" с множественными триггерами

Пример январь 2008:
```
2008-01-21, Верхоянск: Z-score = 3.74 (экстремально тёпло)
                        + экстремальные осадки
                        = 3 risk points, severity: ЭКСТРЕМАЛЬНАЯ
```

---

### **3. Гидрологические данные реки Лена (hydro/*.csv)**

Ежедневные уровни воды в сантиметрах на 5 станциях (семисуточные наблюдения):

| Станция | Координаты | Период | Цель |
|---------|-----------|--------|------|
| **lena-lensk.csv** | Ленск (верховья) | 1998+ | начало половодья |
| **lena-olekminsk.csv** | Олёкминск | 2000+ | промежуточная точка |
| **lena-yakutsk.csv** | Якутск | 2001+ | **основной объект мониторинга** |
| **lena-zhigansk.csv** | Жиганск (низовья) | 2008+ | прогноз наводнений вниз |

**Пример (якутск, май-июнь):**
```
2002-04-15: -248 см (ледостав, уровень ниже ноля)
Май-июнь: +600-800 см (весеннее половодье!)
```

**Модель в `hydro_propagation.py` связывает:**
```
H_yakutsk = -120.0 + 0.38 × H_lensk + 0.42 × H_olekminsk
Лаги добегания пика: 4.4 дня (Ленск→Якутск), 3.8 дня (Якутск→Жиганск)
```

---

### **4. MCHS Emergency Events (mchs_events.csv, mchs_events_lena_bank.csv)**

**Официальная база ЧС по Якутии:**

| Тип события | Частота | Сезон |
|----------|---------|--------|
| **Половодье** | май-июнь | весенний |
| **Затор** (ice jams) | май | весенний |
| **Пожароопасность** | июнь-август | летний |
| **Сильный мороз** | декабрь | зимний |

**Фокус на Лене:**
- 2008-05-24: половодье, Якутск
- 2010-05-20: затор льда, Лена
- 2018-05-14 до 2018-05-27: последовательные заторы на разных участках реки

**Ваша гипотеза:** Связь потепления → деградация мерзлоты → более интенсивное весеннее половодье

---

### **5. WMO Meteorological Files (compressed)**

В `data/air/`, `data/soil/`, `data/snow/` хранятся ZIP-архивы с историческими логами станции **24959 (Якутск/Tuymada)**:

- **wr373144a2** (air): Ежедневные температуры, осадки (дни, где T > 0°C → DDT)
- **wr373144a3** (soil): Температура грунта на разных глубинах, включая **T_3.2м** для модели тренда
- **wr373144a5** (snow): Максимальная высота снежного покрова в зимний период

**Парсятся в `run_dss.py` (lines 102-194)** для извлечения:
- DDT = ∑T где T > 0 (градусо-сутки таяния)
- DDF = |∑T где T < 0| (градусо-сутки замораживания, зима)
- H_snow = max(высота снега Nov-Mar)
- ROS = дни с оттепелью в ноябре-марте
...
```
┌─────────────────────────────────────────────────────┐
│         Климатические метеорологические данные      │
│  (DDT, DDF, H_snow, T_air, осадки из WMO логов)    │
└────────────┬────────────────────────────────────────┘
             │
             ├──→ ALT Models (Stefan, Empirical)
             │    ↓
             │    Глубина сезонного протаивания
             │
             ├──→ Soil Thermal Trend (T_3.2м)
             │    ↓
             │    Потепление вечной мерзлоты
             │    (тренд: +2.15°C за 100 лет)
             │
             ├──→ Monthly Anomalies Detection
             │    ↓
             │    Выявление экстремальных месяцев
             │
└────────────┴──→ PSRS Score (0-10)
                  ↓
             MCHS Recommendations:
             - LOW: плановый мониторинг
             - MODERATE: усиленный мониторинг + термосифоны
             - EXTREME: эвакуация + срочные работы
             
Параллельно: Лена River (Lensk, Olek) → Прогноз пика в Якутске
             ↓
             Паводки на Графском берегу, заторы льда
             ↓
             MCHS警告 (Warning threshold: 700-850 см)
```

---

## 📈 Ключевые числа для Якутска (R42, 1990-2024)

| Метрика | Значение | Источник |
|---------|----------|----------|
| **ALT среднее** | 221.5 см | CALM R42 |
| **ALT тренд** | ≈ +0.3 см/год (слабый) | данные |
| **T_3.2м в 1970е** | ≈ -1.3°C | OLS fit |
| **T_3.2м в 2024** | ≈ -0.45°C | OLS trend |
| **Критический год** | ~2030 | когда T_3.2м = 0°C |
| **Снежный покров зимой** | 35.4 см (норма) | ±8.6 см |
| **DDT среднее** | ~1950 °C-дней | метеоданные |
| **Лена пик, Якутск** | 600-800 см | половодье май-июнь |
...
```
data/
├── СТС.csv                          # CALM ALT measurements (R42 + другие площадки)
├── monthly_anomalies_operational.csv # Месячные аномалии (T, осадки, оттепели)
├── daily_anomalies_operational.csv   # Суточные аномалии (экстремумы, потепления)
├── mchs_events.csv                   # Все ЧС по Якутии (1991-2024)
├── mchs_events_lena_bank.csv         # ЧС только на Лене
├── monthly_anomalies_operational.csv # Месячные аномалии (T, осадки, оттепели)
├── daily_anomalies_operational.csv   # Суточные аномалии
├── air/
│   └── wr373144a2.zip               # WMO метеоданные станции 24959 (Якутск)
├── soil/
│   └── wr373144a3.zip               # WMO почвенные температуры
├── snow/
│   └── wr373144a5.zip               # WMO снежный покров
├── hydro/
│   ├── lena-lensk.csv               # Уровни воды, Ленск
│   ├── lena-olekminsk.csv           # Уровни воды, Олёкминск
│   ├── lena-yakutsk.csv             # Уровни воды, Якутск
│   ├── lena-zhigansk.csv            # Уровни воды, Жиганск
│   ├── lena-kirensk.csv
│   ├── lena-pokrovsk.csv
│   ├── lena-sangar.csv
│   └── lena-vitim.csv
└── permafrost_model/
    ...
    ├── alt_solver.py                # ALT prediction models
    ├── thermal_trend.py             # T_3.2м trend analysis
    ├── hydro_propagation.py         # River peak forecasting
    └── dss.py                       # Decision Support System + PSRS Score
```
...
**Связь между деградацией вечной мерзлоты и гидрологическими рисками на Лене:**

1. **Глобальное потепление** → T_3.2м растёт (+2.15°C за век)
2. **Деградация мерзлоты** → ALT увеличивается, грунт теплеет
3. **Изменение снежного покрова** → ROS растёт (больше зимних оттепелей)
4. **Отток вод весной** → более интенсивное половодье на Лене
5. **MCHS риски** → затопления, заторы, эвакуации

**Модель PSRS** интегрирует всё это в единый 0-10 балл для принятия решений.

---

## 💡 Что дальше можно исследовать

- Корреляция между ROS (зимние оттепели) и пиками половодья
- Влияние H_snow на интенсивность весеннего стока
- Временные лаги между аномалиями температуры и событиями ЧС
- Валидация PSRS-score на исторических 2008, 2010, 2013 годах (максимальные паводки)
- Сценарное моделирование на 2030-2050 под разными RCP сценариями
