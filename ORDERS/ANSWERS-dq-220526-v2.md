...
1. Для каждого дня вычисляется отклонение наблюдаемого значения от опорной климатической нормы (или от соответствующего базового распределения за 2008–2023 гг.).
...
3. Классификация строится по статистическим порогам, заданным в `permafrost_analysis.ipynb` (z-оценки/квантили для экстремальности и признак перехода через 0 °C для оттепелей); в данном тексте эти пороги фиксируются на уровне методического принципа, а не числовых констант.
