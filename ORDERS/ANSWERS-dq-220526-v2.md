...
Если $z_{\max}=z_{\min}$, ряд является константным и не используется для дифференцирующего LC-анализа, поскольку не содержит фазового смещения. После этого для нормированного ряда $v_t$ из $n$ равноотстоящих наблюдений строится LC-кривая как отношение суммы значений от текущего момента $t$ до конца интервала $n$ к полной сумме ряда, то есть как доля эффекта, «остающегося» в хвосте ряда после момента $t$:
...
Такая процедура, непосредственно следующая методике PDF-источника, подавляет малые флуктуации и выделяет именно те участки, где происходит концентрация риска.
