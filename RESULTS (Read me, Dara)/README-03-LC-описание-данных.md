...
Параметры температуры воздуха и осадков получены из исходного ряда [data/air/wr373144a2/wr373144a2.txt](../data/air/wr373144a2/wr373144a2.txt) с суточным шагом. Источник покрывает 6 WMO-станций (Верхоянск, Жиганск, Оймякон, Олёкминск, Якутск, Киренск) за 1882-2025; для сопоставимого анализа аномалий принят расчетный период 2008-2023. По этим данным построены суточные и месячные аномальные витрины [RESULTS (Read me, Dara)/data_tables/daily_anomalies_operational.csv](data_tables/daily_anomalies_operational.csv) и [RESULTS (Read me, Dara)/data_tables/monthly_anomalies_operational.csv](data_tables/monthly_anomalies_operational.csv), а также обзорный график аномалий [RESULTS (Read me, Dara)/plots/anomalies_overview.png](plots/anomalies_overview.png). Статистический профиль исходных значений теперь рассчитан по станциям и параметрам в [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv): например, для Якутска среднесуточная температура имеет медиану -5.6 °C, среднее -9.36 °C, P05 -45.3 °C, P95 20.9 °C, а суточные осадки имеют P95 3.0 мм и максимум 48.5 мм. В расчетах используются z-оценки, пороги P95/P99, интегральные баллы risk_points и категория severity, что позволяет одновременно оценивать уровень, разброс и экстремальность температурно-осадочных сигналов.

### Снежный покров

Параметры снежного покрова (высота снега и производные зимние показатели) получены из ряда [data/snow/wr373144a5/wr373144a5.txt](../data/snow/wr373144a5/wr373144a5.txt) с суточным шагом. Источник покрывает те же 6 WMO-станций за 1882-2025; для анализа принята сквозная сезонная логика зимы (октябрь-апрель) и период сопоставления 2008-2023. На основе этих данных рассчитываются max_snow_depth, winter_thaw_days и ROS-сигналы, которые входят в оперативные и годовые индикаторы риска. Распределение высоты снега профилировано по станциям в [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv): для Якутска медиана по всем суточным наблюдениям равна 0 см, среднее 10.43 см, P95 36 см, максимум 61 см; нулевые значения отражают бесснежную часть года. Качество и полнота источников зафиксированы в [RESULTS (Read me, Dara)/data_profiles/source_catalog.csv](data_profiles/source_catalog.csv) и [RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv).

### Температура грунта

Параметры температуры грунта по глубинам получены из ряда [data/soil/wr373144a3/wr373144a3.txt](../data/soil/wr373144a3/wr373144a3.txt) с суточным шагом. Источник покрывает 6 WMO-станций за 1963-2024 и глубины 2, 5, 10, 15, 20, 40, 60, 80, 120, 160, 240 и 320 см. Эти данные используются для мерзлотного контура (оценка трендов прогрева, интерпретация устойчивости оснований, калибровка связей с сезонным протаиванием). В [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv) рассчитаны статистики по каждой станции и глубине; для Якутска на глубине 320 см за 1964-2024 получено n=20469, медиана -0.6 °C, среднее -0.59 °C, P95 0.5 °C, максимум 3.8 °C, стандартное отклонение 0.62 °C. Визуальные результаты представлены через графики деградации мерзлоты в папке [plots](plots), а в расчетной части включены в сценарный контур через агрегированные показатели. Источник и его атрибуты отражены в [RESULTS (Read me, Dara)/data_profiles/source_catalog.csv](data_profiles/source_catalog.csv).

### Уровни воды по гидропостам Лены

Гидрологические параметры получены из суточных рядов [data/hydro/lena-*.csv](../data/hydro) для 9 гидропостов ленского профиля (Киренск, Витим, Ленск, Олёкминск, Покровск, Якутск, Табага, Сангар, Жиганск). Для сопоставимого анализа используется период 2008-2023, а в витрине сформированы [RESULTS (Read me, Dara)/data_tables/water_level_annual_features.csv](data_tables/water_level_annual_features.csv), [RESULTS (Read me, Dara)/data_tables/water_level_monthly_corr.csv](data_tables/water_level_monthly_corr.csv), [RESULTS (Read me, Dara)/data_tables/water_level_peak_lags_yakutsk.csv](data_tables/water_level_peak_lags_yakutsk.csv) и [RESULTS (Read me, Dara)/data_tables/lena_lag.csv](data_tables/lena_lag.csv). В исходном профиле [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv) уровни воды рассчитаны по каждому посту; для Якутска исходный суточный ряд имеет период 2001-2026, n=6930, медиану -55 см, среднее 5.09 см, P95 459.55 см, максимум 858 см и стандартное отклонение 245.46 см. Для витринной сводки весенних пиков зафиксированы максимум 854 см и среднее около 648.6 см, а пространственные связи между постами оцениваются корреляционной матрицей и лаговыми характеристиками. Графики: [RESULTS (Read me, Dara)/plots/water_level_spring_peaks.png](plots/water_level_spring_peaks.png) и [RESULTS (Read me, Dara)/plots/water_level_peak_lags_yakutsk.png](plots/water_level_peak_lags_yakutsk.png).

### Глубина сезонного протаивания (СТС/ALT)

Параметры глубины сезонного протаивания получены из исходной таблицы [data/СТС.csv](../data/%D0%A1%D0%A2%D0%A1.csv) в широком годовом формате (колонки 1990-2025). На витрине используется [RESULTS (Read me, Dara)/data_tables/СТС.csv](data_tables/%D0%A1%D0%A2%D0%A1.csv). Для этого источника в профилях рассчитаны распределения по каждому году, включая min, p05, median, mean, p95, max, std и IQR: [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv). Полнота сильно неоднородна по площадкам и отражена в [RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv), что учитывается при интерпретации межгодовых трендов и сценарных выводов.

### Архивы чрезвычайных ситуаций (ЧС)

Событийные данные ЧС представлены источниками [data/mchs_events.csv](../data/mchs_events.csv) и [data/mchs_events_lena_bank.csv](../data/mchs_events_lena_bank.csv) с событийным шагом (дата начала/окончания, тип, локация). В расчетной витрине используются [RESULTS (Read me, Dara)/data_tables/mchs_events.csv](data_tables/mchs_events.csv) и [RESULTS (Read me, Dara)/data_tables/mchs_events_lena_bank.csv](data_tables/mchs_events_lena_bank.csv). Полный архив [data/mchs_events.csv](../data/mchs_events.csv) содержит 149 событий за 1991-2024, из них 79 событий попадают в расчетное окно 2008-2023; ленская береговая выборка содержит 18 событий за 2008-2023. Для событий профилируются длительность события и ущерб damage_rub, но ущерб заполнен не для всех записей, поэтому в [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv) он рассматривается как частично заполненный количественный показатель. Годовая событийная полнота отражена в [RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv), а связка «аномалия -> ЧС» зафиксирована в [RESULTS (Read me, Dara)/data_tables/anomaly_chs_linked.csv](data_tables/anomaly_chs_linked.csv).
...
3. [RESULTS (Read me, Dara)/reproduction_scripts/profile_raw_data.py](reproduction_scripts/profile_raw_data.py) — воспроизводимое профилирование источников, статистик значений и полноты.
4. [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py) — упаковка результатов в витрину data_tables/plots.
5. [RESULTS (Read me, Dara)/permafrost_analysis_notebook.ipynb](permafrost_analysis_notebook.ipynb) — проверка и интерпретация витринных таблиц.
...
2. Для статистических характеристик и полноты введена отдельная витрина профилей:
	[RESULTS (Read me, Dara)/data_profiles/source_catalog.csv](data_profiles/source_catalog.csv),
	[RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv),
	[RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv),
	[RESULTS (Read me, Dara)/data_profiles/transformations_log.csv](data_profiles/transformations_log.csv).
...
4. Слой упаковки в витрину: перенос итоговых таблиц/графиков в [RESULTS (Read me, Dara)/data_tables](data_tables) и [RESULTS (Read me, Dara)/plots](plots) через [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).
5. Слой верификации и интерпретации витрины: [RESULTS (Read me, Dara)/permafrost_analysis_notebook.ipynb](permafrost_analysis_notebook.ipynb).
6. Слой профильной витрины качества источников: [RESULTS (Read me, Dara)/data_profiles](data_profiles).
...
3. [RESULTS (Read me, Dara)/permafrost_analysis_notebook.ipynb](permafrost_analysis_notebook.ipynb)
...
1. [RESULTS (Read me, Dara)/reproduction_scripts/explore_data.py](reproduction_scripts/explore_data.py)
	Первичная разведка структуры исходных файлов.
2. [RESULTS (Read me, Dara)/reproduction_scripts/inspect_columns.py](reproduction_scripts/inspect_columns.py)
	Проверка и унификация колонок/полей.
3. [RESULTS (Read me, Dara)/reproduction_scripts/inspect_anomalies.py](reproduction_scripts/inspect_anomalies.py)
	Диагностика и обзор аномалий/ЧС.
4. [RESULTS (Read me, Dara)/reproduction_scripts/match_alt_stations.py](reproduction_scripts/match_alt_stations.py)
	Сопоставление площадок ALT/СТС со станциями.
5. [RESULTS (Read me, Dara)/reproduction_scripts/inspect_specific_sites.py](reproduction_scripts/inspect_specific_sites.py)
	Локальная проверка выбранных площадок/станций.
6. [RESULTS (Read me, Dara)/reproduction_scripts/inspect_sample_data.py](reproduction_scripts/inspect_sample_data.py)
	Контроль корректности примеров исходных рядов.
7. [RESULTS (Read me, Dara)/reproduction_scripts/inspect_fixed_width.py](reproduction_scripts/inspect_fixed_width.py)
	Проверка fixed-width форматов.
8. [RESULTS (Read me, Dara)/reproduction_scripts/test_fixed_slices.py](reproduction_scripts/test_fixed_slices.py)
	Тестирование правил разреза fixed-width строк.
9. [RESULTS (Read me, Dara)/reproduction_scripts/inspect_met_coverage.py](reproduction_scripts/inspect_met_coverage.py)
	Оценка метеорологического покрытия по станциям/периодам.
10. [RESULTS (Read me, Dara)/reproduction_scripts/profile_raw_data.py](reproduction_scripts/profile_raw_data.py)
	Воспроизводимое формирование source_catalog, value_stats_raw и completeness_report для исходных данных.
11. [RESULTS (Read me, Dara)/reproduction_scripts/read_mes.py](reproduction_scripts/read_mes.py)
	 Утилитарное чтение и разбор вспомогательных текстовых материалов.
12. [RESULTS (Read me, Dara)/reproduction_scripts/train_alt_model.py](reproduction_scripts/train_alt_model.py)
	 Обучение/оценка моделей ALT.
13. [RESULTS (Read me, Dara)/reproduction_scripts/calibrate_stefan.py](reproduction_scripts/calibrate_stefan.py)
	 Калибровка моделей Стефана.
14. [RESULTS (Read me, Dara)/reproduction_scripts/analyze_soil_warming.py](reproduction_scripts/analyze_soil_warming.py)
	 Анализ трендов потепления грунта.
15. [RESULTS (Read me, Dara)/reproduction_scripts/analyze_hydro.py](reproduction_scripts/analyze_hydro.py)
	 Анализ гидропиков и лагов прохождения волны.
16. [RESULTS (Read me, Dara)/reproduction_scripts/inspect_existing_results.py](reproduction_scripts/inspect_existing_results.py)
	 Проверка структуры сформированных результатов.
17. [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py)
	 Витринная упаковка: перенос таблиц/графиков в каталог RESULTS.
18. [RESULTS (Read me, Dara)/reproduction_scripts/find_ipynb.py](reproduction_scripts/find_ipynb.py)
...
## A2.1 Методическая преемственность со статьей по LC-кривым

Методика данного раздела прямо наследует и развивает подход из статьи [ORDERS/статья_LC_финал_11_08.pdf](../ORDERS/%D1%81%D1%82%D0%B0%D1%82%D1%8C%D1%8F_LC_%D1%84%D0%B8%D0%BD%D0%B0%D0%BB_11_08.pdf), где LC-кривые используются как способ анализа временных рядов, выявления стадий, отклонений, шоков и поворотных точек. В статье LC понимается как Life Cycle Curves, а в данном проекте расчетный контур расширен до Lead-Lag Correlation. Это не отказ от исходной методики, а ее доменная адаптация: вместо отраслевых траекторий деловой неопределенности рассматриваются природные и событийные траектории риска, а вместо сравнения отраслей — сравнение предикторов и откликов во времени.

Преемственность выражается в следующих элементах:

1. **Временной ряд как базовая единица анализа.** В статье LC-кривая строится по нормированному временному ряду показателя; в README-03 каждая климатическая, гидрологическая, мерзлотная и событийная витрина также приводится к временным рядам с дневным, месячным или годовым шагом.
2. **Нормировка и сопоставимость рядов.** Статья требует нормировать ряды для сравнения объектов; в README-03 это развивается через z-оценки, квантили P95/P99, ранги, risk_points и severity, чтобы сравнивать разные физические параметры: температуру, осадки, снег, уровни воды и события ЧС.
3. **Выявление отклонений и шоков.** В статье LC-кривые фокусируют анализ на значимых отклонениях от равномерной траектории и чувствительны к шокам; в README-03 это перенесено в язык природных аномалий: экстремальные осадки, резкие оттепели, пики половодья, превышения порогов и всплески событий ЧС.
4. **Сравнение траекторий разных объектов.** В статье сравниваются отрасли и подотрасли; в README-03 сравниваются гидропосты Лены, метеостанции, площадки СТС и типы ЧС. Тем самым сохраняется принцип многоконтурного сравнения, но объект анализа меняется с экономического на природно-управленческий.
5. **Переход от описания к оперативной интерпретации.** В статье LC-кривые предлагаются для мониторинга, сценарного анализа и выявления поворотных точек; в README-03 этот принцип доведен до управленческого вывода: если предиктор устойчиво опережает отклик, он задает окно упреждения для реагирования МЧС.

Таким образом, связь со статьей является не только терминологической. README-03 сохраняет методическое ядро статьи: временной ряд -> нормировка -> выявление отклонений/аномалий -> сравнение траекторий -> интерпретация устойчивости и поворотных точек. Развитие состоит в том, что для задач природного риска к этому ядру добавлены явные лаги $k$, пары $x_t/y_t$, гидрологическое добегание волны, связь «аномалия -> ЧС» и сценарная интерпретация для принятия решений.
...
1. [RESULTS (Read me, Dara)/data_profiles/source_catalog.csv](data_profiles/source_catalog.csv) — паспорт источников.
2. [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv) — распределения и разброс по исходным параметрам.
3. [RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv) — полнота по годам/площадкам.
4. [RESULTS (Read me, Dara)/data_profiles/transformations_log.csv](data_profiles/transformations_log.csv) — журнал преобразований source -> data_tables.

## A4.1 Файлы оперативной аномалистики (вход в LC климат->ЧС)

### daily_anomalies_operational.csv (3095 строк)

Формируется в: [NOPik/permafrost_analysis.ipynb](../NOPik/permafrost_analysis.ipynb). Витринный перенос в data_tables: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).

Статистические характеристики исходных значений (до агрегирования в витрину) см. в [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv) и полноту по времени в [RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv).
...
Формируется в: [NOPik/permafrost_analysis.ipynb](../NOPik/permafrost_analysis.ipynb) (конвертация и фильтрация XLS-архива ЧС). Витринный перенос в data_tables: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).

Статистический профиль событий и годовая полнота см. в [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv) и [RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv).
...
Формируется в: [NOPik/permafrost_analysis.ipynb](../NOPik/permafrost_analysis.ipynb) (фильтр «берег Лены» по событиям ЧС). Витринный перенос в data_tables: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).
...
Формируется в: [NOPik/permafrost_analysis.ipynb](../NOPik/permafrost_analysis.ipynb) (увязка аномалий с ЧС в лаговых окнах). Витринный перенос в data_tables: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).
...
Формируется в: [NOPik/permafrost_analysis.ipynb](../NOPik/permafrost_analysis.ipynb) (блок анализа гидропостов, расчет годовых признаков). Витринный перенос в data_tables: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).

Распределения исходных уровней воды и полнота суточных рядов по годам см. в [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv) и [RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv).
...
Формируется в: [NOPik/permafrost_analysis.ipynb](../NOPik/permafrost_analysis.ipynb) (тот же расчет лагов, вариант имени файла). Витринный перенос в data_tables: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).
...
Формируется в: [NOPik/permafrost_analysis.ipynb](../NOPik/permafrost_analysis.ipynb) (корреляция среднемесячных уровней между постами). Витринный перенос в data_tables: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).
...
Формируется в: [NOPik/permafrost_analysis.ipynb](../NOPik/permafrost_analysis.ipynb) (лаги по датам оттепели/пиков относительно Якутска). Витринный перенос в data_tables: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).
...
Формируется в: [NOPik/permafrost_analysis.ipynb](../NOPik/permafrost_analysis.ipynb) (годовые индексы и сценарная классификация). Витринный перенос в data_tables: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).
...
Источник: [data/СТС.csv](../data/%D0%A1%D0%A2%D0%A1.csv). В data_tables файл переносится напрямую скриптом [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py) без пересчета.

Статистика значений по годовым колонкам и полнота по площадкам см. в [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv) и [RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv).
...
3. [RESULTS (Read me, Dara)/permafrost_analysis_notebook.ipynb](permafrost_analysis_notebook.ipynb) — витринный ноутбук проверки и интерпретации уже сформированных таблиц из data_tables.

### 11.2 Ключевые скрипты воспроизведения

1. [RESULTS (Read me, Dara)/reproduction_scripts/analyze_hydro.py](reproduction_scripts/analyze_hydro.py) — расчет пиков половодья и лагов добегания волны.
2. [RESULTS (Read me, Dara)/reproduction_scripts/inspect_anomalies.py](reproduction_scripts/inspect_anomalies.py) — проверка и сводка аномалий и ЧС.
3. [RESULTS (Read me, Dara)/reproduction_scripts/profile_raw_data.py](reproduction_scripts/profile_raw_data.py) — пересчет профилей источников, статистики значений и полноты.
4. [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py) — перенос расчетных CSV в data_tables и графиков в plots.
5. [RESULTS (Read me, Dara)/reproduction_scripts/inspect_existing_results.py](reproduction_scripts/inspect_existing_results.py) — контроль структуры итоговых CSV.
...
7. Добавлен воспроизводимый скрипт [RESULTS (Read me, Dara)/reproduction_scripts/profile_raw_data.py](reproduction_scripts/profile_raw_data.py), который формирует расширенные профили источников.

### 12.2 Что создано на витрине data_profiles

Создана витрина профилирования исходных данных:
[RESULTS (Read me, Dara)/data_profiles](data_profiles)

Сформированы первые отчеты:

1. [RESULTS (Read me, Dara)/data_profiles/source_catalog.csv](data_profiles/source_catalog.csv) — 7 строк.
	Содержит паспорт источников: группа, путь, формат, шаг времени, география, параметры, единицы, примечания.
2. [RESULTS (Read me, Dara)/data_profiles/value_stats_raw.csv](data_profiles/value_stats_raw.csv) — 139 строк.
	Содержит статистики по исходным числовым значениям: n_non_null, n_null, min, p05, median, mean, p95, max, std, iqr; профили рассчитаны для air, snow, soil, hydro, СТС и ЧС.
3. [RESULTS (Read me, Dara)/data_profiles/completeness_report.csv](data_profiles/completeness_report.csv) — 6951 строка.
	Содержит первичную оценку полноты: по станциям/годам/параметрам для air, snow, soil; по годам для гидропостов; по площадкам/годовым колонкам для СТС; и count-only по годам для ЧС.
4. [RESULTS (Read me, Dara)/data_profiles/transformations_log.csv](data_profiles/transformations_log.csv) — 12 строк.
...
2. Упаковка результатов в витрину data_tables/plots: [RESULTS (Read me, Dara)/reproduction_scripts/copy_results.py](reproduction_scripts/copy_results.py).
3. Проверка и обзор итоговых таблиц: [RESULTS (Read me, Dara)/permafrost_analysis_notebook.ipynb](permafrost_analysis_notebook.ipynb).

### 12.4 Ограничения текущей версии профилей

1. Текущий value_stats_raw профилирует fixed-width ряды air/snow/soil как числовые панели по станциям и параметрам, но не заменяет полноценный климатологический контроль качества исходных WMO-бюллетеней.
