...
```mermaid
graph TD
    A["ЧС в криолитозоне Якутии"] --> B["Гидрологические ЧС"]
    A --> C["Геотехнические ЧС оснований"]
    
    B --> B1["Весенние ледовые заторы"]
    B --> B2["Дождевые паводки (лето/осень)"]
    B --> B3["Термоэрозия речных берегов"]
    
    C --> C1["Просадка фундаментов зданий"]
    C --> C2["Разрушение дорог и ЛЭП"]
    C --> C3["Аварии резервуаров ТЭК"]
```
...
![Вековой тренд потепления мерзлоты на глубине 3.2м](file:///C:/Diploma/permafrost_analysis/RESULTS%20%28Read%20me,%20Dara%29/plots/anomalies_overview.png)
...
graph TD
    ...
    C -->|4.0 <= PSRS < 7.0| E["Желтый уровень: Готовность"]
    C -->|PSRS >= 7.0| F["Красный уровень: Режим ЧС"]
    
    D --> D1["Термопрофилирование свай"]
    E --> E1["Откачка воды и охлаждение свай"]
    F --> F1["Эвакуация населения и срезка льда"]
```
